...
infra entity (loki, eks, aurora, ...), we look it up in `.claude/graph.json`
...
# Entities we expect to find as nodes in `.claude/graph.json`. Subset of
...
def _graph_path() -> str:
    """Locate `.claude/graph.json` relative to CLAUDE_PROJECT_DIR or cwd."""
    root = os.environ.get("CLAUDE_PROJECT_DIR") or os.getcwd()
    return os.path.join(root, ".claude", "graph.json")
