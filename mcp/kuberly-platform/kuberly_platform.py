...
class KuberlyPlatform:
    ...
    def scan_state_overlays(self):
        ...
        from `.claude/state_overlay_*.json` files generated by
        `state_graph.py`.

        The static graph alone underreports: modules like loki / grafana /
        alloy can be deployed without a `components/<env>/<m>.json` shim,
        and the JSON-sidecar actionability check then incorrectly flags
        them as `stop-no-instance`. The overlay closes that gap by
        synthesizing `component:<env>/<m>` nodes for every module found
        in the state bucket. Synthetic nodes are tagged `source: "state"`
        so reports can distinguish "declared in components/" from
