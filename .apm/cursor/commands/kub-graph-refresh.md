...
3. Tell the operator how to open **`graph.html`** locally and what to expect after **v0.33.0+**: **concentric · 3D** presentation, **Graph scope → Overview (module deps)** for large stacks (Terragrunt **module** + **`depends_on`** only); **Full graph** for deep dives; choice persists in **sessionStorage**.
