...
5. If **kuberly-platform MCP** is available, optionally call **`query_nodes`** for `type:environment` or `shared-infra` to echo **account / region / cluster_name** from the graph — treat as advisory vs live AWS. Beyond the four classic layers (`static` / `state` / `k8s` / `docs`), the graph also exposes **`schema`** (`cue_schema` nodes — `cue/**/*.cue`), **`ci_cd`** (`workflow` nodes — `.github/workflows/*.yml` + `references` edges to module/component), and **`rendered`** (`app_render` umbrella + `rendered_resource` leaves, populated by manual `scripts/render_apps.py`). Use those when answering "which workflow deploys this" / "what does this app render into".
